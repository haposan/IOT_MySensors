...
{
	FLASH_read_at(CY_SFLASH_USERBASE + (uint32_t)pos, (uint8_t *)buffer, length);
}
...
void hwWriteConfigBlock(void *buffer, const void *pos, size_t length)
{
	...
	for (int i = 0; i < length; i++)
	{
		...
		if (buf[i] != flash[i])
		{
			FLASH_update(CY_SFLASH_USERBASE + (uint32_t)pos, (uint8_t *)buffer, length);
			return;
		}
	}
}
...
{
	BoardGetUniqueId((uint8_t *)uniqueID);
	(void)memset((uint8_t *)uniqueID + 8, MY_HWID_PADDING_BYTE, 8);
	return true;
}
